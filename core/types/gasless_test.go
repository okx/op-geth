...
func TestGaslessAddressFor(t *testing.T) {
	cases := []struct {
		...
		{"chain_196", big.NewInt(196), common.HexToAddress("0x420000000000000000000000000000000000002F")},
...
func TestDecodeGaslessEnabled(t *testing.T) {
	zero := make([]byte, 32)
	one := make([]byte, 32)
	one[31] = 1
	cases := []struct {
		...
		{"too_short", make([]byte, 16), false, true},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got, err := DecodeGaslessEnabled(tc.in)
			if tc.wantError {
				if err == nil {
					t.Fatalf("expected error, got %v", got)
				}
