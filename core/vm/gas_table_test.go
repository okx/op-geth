...
var createGasTestsErigon = []struct {
	code    string
	eip3860 bool
	gasUsed uint64
}{
...
func TestCreateGasErigon(t *testing.T) {
	for i, tt := range createGasTestsErigon {
		address := common.BytesToAddress([]byte("contract"))
		...
		vmenv := NewEVM(vmctx, statedb, params.TestChainConfig, config)

		var startGas uint64 = math.MaxUint64
		_, gas, err := vmenv.Call(common.Address{}, address, nil, startGas, new(uint256.Int))
		if err != nil {
