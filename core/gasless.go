...
// IsGaslessEnabled invokes the gaslessEnabled() auto-generated getter on the
// Gasless predeploy and returns its boolean answer. Failures (no code at the
// predeploy, call revert, malformed return) are absorbed and reported as
// false so the caller falls back to the strict, non-gasless code path.
func IsGaslessEnabled(evm *vm.EVM) bool {
	predeploy := types.GaslessAddressFor(evm.ChainConfig().ChainID)
	if len(evm.StateDB.GetCode(predeploy)) == 0 {
		return false
	}
	...
	ret, _, err := evm.Call(params.SystemAddress, predeploy, types.GaslessEnabledCalldata(), gaslessProbeGasLimit, common.U2560)
	if err != nil {
		return false
	}
	enabled, err := types.DecodeGaslessEnabled(ret)
	if err != nil {
		return false
	}
	return enabled
