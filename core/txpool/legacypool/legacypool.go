...
type Config struct {
	...
	// EnableGasless controls whether the pool participates in the Gasless flow:
...
// zero-fee gasless LazyTransactions exported by Pending. When the background
// mock gas price is set, it is used directly. Otherwise the fallback is
// baseFee * 10. Returns nil when gasless is disabled or the resulting value
// is non-positive (so callers leave the LazyTransaction's caps untouched).
func (pool *LegacyPool) gaslessOrderingFee(baseFee *uint256.Int) *uint256.Int {
	if !pool.config.EnableGasless {
		return nil
	}
	...
	fallback := new(uint256.Int).Mul(baseFee, uint256.NewInt(1))
	return fallback
}
...
func (pool *LegacyPool) refreshMockGasPrice(head *types.Header) {
	if !pool.config.EnableGasless || head == nil {
		return
	}
