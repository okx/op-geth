...
type Message struct {
	...
	IsGaslessTx    bool                 // IsGaslessTx indicates the message is exempt from fee deduction (per-tx Gasless predeploy decision).
...
func (st *stateTransition) preCheck() error {
	...
	if st.msg.IsGaslessTx {
		// Gasless transactions: nonce must still be enforced (these are
		...
		// bounded amount of gasless work), then skip buyGas entirely.
		msg := st.msg
...
func (st *stateTransition) innerExecute() (*ExecutionResult, error) {
	...
	} else if st.msg.IsGaslessTx {
		// Gasless transactions: skip all fee distribution (coinbase tip,
