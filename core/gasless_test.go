...
var (
	// returns 32 bytes where byte 31 = 0x01 (i.e. ABI-encoded `true`).
	bcReturnTrueBool = []byte{0x60, 0x01, 0x60, 0x1f, 0x53, 0x60, 0x20, 0x60, 0x00, 0xf3}
	// returns 32 zero bytes (ABI-encoded `false`).
	bcReturnFalseBool = []byte{0x60, 0x20, 0x60, 0x00, 0xf3}
...
func TestIsGaslessEnabled(t *testing.T) {
	cases := []struct {
		name string
		code []byte
		want bool
		...
		{"no_code", nil, false},
		{"returns_true", bcReturnTrueBool, true},
		{"returns_false", bcReturnFalseBool, false},
		{"reverts", bcRevert, false},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			evm := setupGaslessEVM(t, big.NewInt(1), tc.code)
			if got := IsGaslessEnabled(evm); got != tc.want {
...
func TestIsGaslessEnabled_StateIsReverted(t *testing.T) {
	// Confirm that IsGaslessEnabled does NOT leave any statedb residue
	// (its snapshot/revert behaviour). We compare intermediate roots
	// before and after the call.
	evm := setupGaslessEVM(t, big.NewInt(1), bcReturnTrueBool)
	sdb := evm.StateDB.(*state.StateDB)
	preRoot := sdb.IntermediateRoot(false)

	if !IsGaslessEnabled(evm) {
		t.Fatal("expected true")
	}
	postRoot := sdb.IntermediateRoot(false)

	if preRoot != postRoot {
		t.Fatalf("state root mutated: pre=%s post=%s", preRoot.Hex(), postRoot.Hex())
	}
}
