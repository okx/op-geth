...
[Rule] `p2p/transport_xlayer.go`: When remote peer is identified as upstream Geth (client name starts with `Geth/` prefix, checked via `strings.HasPrefix(name, "Geth/")`), ETH69 capability must be stripped before handshake. Fork nodes (`l2-geth`, `op-geth`, etc.) must NOT match. Reason: ETH69 protocol compatibility issue with standard Geth nodes; substring matching previously caused fork-to-fork peering failures.
