...
[Rule] `p2p/transport_xlayer.go`: ETH69 capability must be stripped for Geth peers before handshake (when `eth69CompatEnabled` is true; runtime-disableable via `--p2p.eth69-compat=false`).
