...
[Rule] `p2p/transport_xlayer.go`: When `--p2p.eth69-compat=true` (default), ETH69 capability must be stripped for Geth peers before handshake. Disable with `--p2p.eth69-compat=false` once upstream interop is verified.
