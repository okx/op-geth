...
| `transport_xlayer` | `eth69CompatEnabled`, `eth69TrimCounter`, `SetETH69CompatEnabled`, `TrimOurHandshakeCaps`, `isGeth`, `trimETH69Caps`, `doProtoHandshakeWithXLayerFilter`, `doProtoHandshakeForConn` | XLayer ETH69 two-phase compat layer (Phase A: startup outgoing trim; Phase B: per-peer incoming filter) |
...
[Pitfall] Sequential read-then-send handshake causes protocol-level deadlock — always delegate to upstream concurrent `doProtoHandshake` and post-filter the result. Source: `p2p/transport_xlayer.go` (TD v1→v2 evolution).
