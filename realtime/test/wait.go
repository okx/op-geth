...
func WaitRealtimeTxToBeConfirmed(parentCtx context.Context, client *rtclient.RealtimeClient, tx *types.Transaction, timeout time.Duration) error {
	ctx, cancel := context.WithTimeout(parentCtx, timeout)
	defer cancel()
	receipt, err := WaitMined(ctx, client, tx.Hash())
	if errors.Is(err, context.DeadlineExceeded) {
...
func WaitEthTxToBeConfirmed(parentCtx context.Context, client ethClienter, tx *types.Transaction, timeout time.Duration) error {
	ctx, cancel := context.WithTimeout(parentCtx, timeout)
	defer cancel()
	receipt, err := WaitMined(ctx, client, tx.Hash())
	if errors.Is(err, context.DeadlineExceeded) {
