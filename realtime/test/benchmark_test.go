...
func TestRealtimeBenchmarkNativeTransferConfirmation(t *testing.T) {
	...
	for i := 0; i < Iterations; i++ {
		balance, err := nonRtClient.BalanceAt(ctx, testAddress, nil)
		require.NoError(t, err)
		...
		g.Go(func() error {
			startTime := time.Now()
			err = WaitRealtimeTxToBeConfirmed(ctx, client, signedTx, DefaultTimeoutTxToBeMined)
			require.NoError(t, err)
		...
		g.Go(func() error {
			startTime := time.Now()
			err = WaitEthTxToBeConfirmed(ctx, nonRtClient, signedTx, DefaultTimeoutTxToBeMined)
			require.NoError(t, err)
...
func TestRealtimeBenchmarkNativeTransfer(t *testing.T) {
	...
	for i := 0; i < Iterations; i++ {
		...
		g.Go(func() error {
			duration, err := WaitCallback(ctx, nonRtClient, signedTx, common.Address{}, *signedTx.To(), balance, DefaultTimeoutTxToBeMined, WaitAccountBalanceEth)
			if err != nil {
				return err
			}
		...
		fmt.Printf("Iteration %v:\n", i)
		fmt.Printf("RT native tx transfer state update took: %s\n", realtimeBalanceDuration)
		fmt.Printf("ETH native tx transfer state update took: %s\n", ethBalanceDuration)
	}
	...
	// Log out metrics
	fmt.Printf("Avg RT native tx transfer state update took: %s\n", avgRealtimeBalanceDuration)
	fmt.Printf("Avg ETH native tx transfer state update took: %s\n", avgEthBalanceDuration)
}
...
func TestRealtimeBenchmarkERC20TransferConfirmation(t *testing.T) {
	...
	var totalRealtimeBalanceDuration, totalEthBalanceDuration time.Duration
	for i := 0; i < Iterations; i++ {
		balance, err := nonRtClient.EthGetTokenBalance(ctx, testAddress, erc20Address, nil)
		require.NoError(t, err)
...
func TestRealtimeBenchmarkERC20Transfer(t *testing.T) {
	...
	for i := 0; i < Iterations; i++ {
		...
		g.Go(func() error {
			duration, err := WaitCallback(ctx, nonRtClient, signedTx, fromAddress, testAddress, balance, DefaultTimeoutTxToBeMined, WaitTokenBalanceEth)
			if err != nil {
				return err
			}
			ethBalanceDuration = duration
		...
		fmt.Printf("Iteration %v:\n", i)
		fmt.Printf("RT erc20 tx transfer state update took: %s\n", realtimeBalanceDuration)
		fmt.Printf("ETH erc20 tx transfer state update took: %s\n", ethBalanceDuration)
	}
	...
	// Log out metrics
	fmt.Printf("Avg RT erc20 tx transfer state update took: %s\n", avgRealtimeBalanceDuration)
	fmt.Printf("Avg ETH erc20 tx transfer state update took: %s\n", avgEthBalanceDuration)
}
