...
var (
	...
	RollupEnableGaslessFlag = &cli.BoolFlag{
		Name:     "rollup.enable-gasless",
		Usage:    "Enable gasless (accept 0-price txs gated per-tx by the Gasless predeploy, swap ordering fee with a background-maintained mock price, and start the mock-price maintainer).",
		Category: flags.RollupCategory,
		Value:    false,
	}
...
func setTxPool(ctx *cli.Context, cfg *legacypool.Config) {
	...
	if ctx.IsSet(RollupEnableGaslessFlag.Name) {
		cfg.EnableGasless = ctx.Bool(RollupEnableGaslessFlag.Name)
	}
